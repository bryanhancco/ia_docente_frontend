...
import { ClaseResponse, ContenidoGenerado, FormularioResponseDTO, ArchivoInfo, ArchivosResponse, EstructuraClaseResponse, RecursosEducativosResponse, PreguntaData, apiService } from '../../../lib/api';
          ...
          <div className="layout-content-container flex flex-col max-w-[960px] flex-1">
            ...
            {activeTab === 'info' && clase && (
              <div>
                ...
                {/* Mostrar el ID de la clase para compartir */}
                <div className="px-4 pb-2">
                  <div className="inline-flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-lg px-3 py-1 mb-2">
                    <span className="text-xs text-[#0d80f2] font-semibold">ID de Clase:</span>
                    <span className="text-xs font-mono text-[#0d141c] select-all">{clase.id}</span>
                  </div>
                  <span className="text-xs text-[#49739c]">Comparte este código con tus estudiantes</span>
                </div>
                <div className="p-4">
                ...
                {contenidos.filter(contenido => contenido.tipo_recurso_generado === 'Estructura de Clase').length > 0 ? (
                    ...
                    {contenidos.filter(contenido => contenido.tipo_recurso_generado === 'Estructura de Clase').map((contenido, index) => (
                      <div key={index} className="bg-white rounded-lg border border-[#cedbe8] p-6">
                        <div className="flex justify-between items-start mb-4">
                          <div>
                            <h3 className="text-[#0d141c] text-lg font-semibold">Estructura de Clase</h3>
                  ...
                  <div className="flex items-center justify-center py-12">
                    <div className="text-center">
                      ...
                      <h3 className="text-[#0d141c] text-lg font-semibold mb-2">No hay estructura generada</h3>
                      <p className="text-[#49739c] text-base mb-4">
                        La estructura de clase se genera automáticamente al procesar la clase.
                      </p>
                    </div>
                  </div>
                )}
            ...
            {activeTab === 'contenido' && (
              <div>
                ...
                <div className="mb-8 p-4">
                  <h3 className="text-[#0d141c] text-lg font-semibold mb-4">Presentación PowerPoint</h3>
                  {contenidos.filter(contenido => contenido.tipo_recurso_generado === 'Presentación PowerPoint').length > 0 ? (
                    <div className="space-y-4">
                      {contenidos.filter(contenido => contenido.tipo_recurso_generado === 'Presentación PowerPoint').map((contenido, index) => {
                        // Extraer información de la presentación del contenido
                        ...
                        return (
                          <div key={index} className="bg-white rounded-lg border border-[#cedbe8] p-6">
                            <div className="flex justify-between items-start mb-4">
                              <div>
                                <h4 className="text-[#0d141c] text-lg font-semibold flex items-center gap-2">
                            ...
                            {embedUrl ? (
                              ...
                              <div className="aspect-video bg-gray-100 rounded-lg flex items-center justify-center">
                                <div className="text-center">
                                  <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="currentColor" viewBox="0 0 256 256" className="mx-auto mb-2 text-gray-400">
                                    <path d="M216,40H136V24a8,8,0,0,0-16,0V40H40A16,16,0,0,0,24,56V176a16,16,0,0,0,16,16H79.36L57.75,219a8,8,0,0,0,12.5,10l29.59-37h56.32l29.59,37a8,8,0,1,0,12.5-10l-21.61-27H216a16,16,0,0,0,16-16V56A16,16,0,0,0,216,40Zm0,136H40V56H216V176Z"/>
                                  </svg>
                    ...
                    <div className="flex items-center justify-center py-8">
                      <div className="text-center">
                        ...
                        <p className="text-[#49739c] text-base">No hay presentación generada</p>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="mb-8 p-4">
                  <h3 className="text-[#0d141c] text-lg font-semibold mb-4">Recursos Educativos Web</h3>
                  {contenidos.filter(contenido => contenido.tipo_recurso_generado === 'Recursos Educativos Web').length > 0 ? (
                    <div className="space-y-4">
                      {contenidos.filter(contenido => contenido.tipo_recurso_generado === 'Recursos Educativos Web').map((contenido, index) => (
                        <div key={index} className="bg-white rounded-lg border border-[#cedbe8] p-6">
                          <div className="flex justify-between items-start mb-4">
                            <div>
                              <h4 className="text-[#0d141c] text-lg font-semibold flex items-center gap-2">
                              ...
                              <p className="text-[#49739c] text-sm">
                                Encontrados el {getFormattedDate(contenido.created_at)}
                              </p>
                            </div>
                            <button
                              onClick={() => downloadAsPDF(contenido)}
                          ...
                          <div className="prose max-w-none">
                            <div className="whitespace-pre-line text-[#0d141c] text-sm leading-relaxed max-h-96 overflow-y-auto bg-gray-50 p-4 rounded-lg">
                              {contenido.contenido}
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="flex items-center justify-center py-8">
                      <div className="text-center">
                        ...
                        <p className="text-[#49739c] text-base">No hay recursos web encontrados</p>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="mb-8 p-4">
                  <h3 className="text-[#0d141c] text-lg font-semibold mb-4">Preguntas de Evaluación</h3>
                  {preguntas.length > 0 ? (
                    <div className="bg-white rounded-lg border border-[#cedbe8] p-6">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <button
                          ...
                          className="bg-[#0d80f2] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-600 flex items-center gap-2"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 256 256">
                            <path d="M128,24A104,104,0,1,0,232,128,104.11,104.11,0,0,0,128,24Zm0,192a88,88,0,1,1,88-88A88.1,88.1,0,0,1,128,216Zm12-88a12,12,0,1,1-12-12A12,12,0,0,1,140,128Zm-12-64a12,12,0,1,0,12,12A12,12,0,0,0,128,64Z"/>
                          </svg>
                          Ver Evaluación
                        </button>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {preguntas.slice(0, 3).map((pregunta, index) => (
                          <div key={index} className="bg-gray-50 rounded-lg p-4">
                            <p className="text-sm font-medium text-[#0d141c] mb-2">
                              Pregunta {index + 1}
                            </p>
                            <p className="text-sm text-[#49739c] line-clamp-3">
                              {pregunta.pregunta}
                            </p>
                          </div>
                        ))}
                      ...
                      {preguntas.length > 3 && (
                        <div className="mt-4 text-center">
                          <p className="text-sm text-[#49739c]">
                            Y {preguntas.length - 3} preguntas más...
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-center py-8">
                      <div className="text-center">
                        ...
                        <p className="text-[#49739c] text-base">No hay preguntas generadas</p>
                      </div>
                    </div>
                  )}
                </div>
